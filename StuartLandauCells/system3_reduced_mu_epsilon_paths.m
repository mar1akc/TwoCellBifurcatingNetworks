function system3_reduced_mu_epsilon_paths()
...
if flag == 1
    ...
    lam = 1;
    k = 500;
    muvals = linspace(1e-3, 10,Nmu);
    for i = 1 : Nep
        for j = 1 : Ns
            ind = find(muvals+ep(i) > 0);
            mu = (((muvals(ind)+ep(i)).^1.5)./sqrt(muvals(ind)))/lam;
            sigma = (s(j)/lam)*sqrt((muvals(ind)+ep(i))./muvals(ind));
            plot(sigma,mu,'Linewidth',lwt1,'color',[i/Nep,0.5,j/Ns]);
            itext = k-(i + j -4)^2*5;
            % if itext > 0
            %     text(sigma(k-(i + j -4)^2*5),mu(k-(i +j -4)^2*5),sprintf("%.1f, %.1f",s(j),ep(i)),...
...
if flag == 2
    ...
    Ns = 1000;
    lam = 1;
    svals = linspace(-3,3,Ns);
    for i = 1 : Nep
        for j = 1 : Nmu
            if m(j) + ep(i) >= 0
                mu = (((m(j)+ep(i)).^1.5)./sqrt(m(j)))/lam;
                sigma = (svals/lam)*sqrt((m(j)+ep(i))./m(j));
                plot(sigma,mu*ones(size(sigma)),'Linewidth',lwt1,'color',[i/Nep,j/Nmu,0.5]);
            end
        end
    end
...
if flag == 3
    ...
    lam = 1;
    k = 300;
    epvals = linspace(-3,3,Nep);
    for i = 1 : Ns
        for j = 1 : Nmu
            ind = find(m(j)+epvals > 0);
            mu = (((m(j)+epvals(ind)).^1.5)./sqrt(m(j)))/lam;
            sigma = (s(i)/lam)*sqrt((m(j)+epvals(ind))./m(j));
            plot(sigma,mu,'Linewidth',lwt1,'color',[j/Nmu,0.5,i/Ns]);

            text(sigma(k-(i + j -4)^2*5),mu(k-(i +j -4)^2*5),sprintf("%.1f, %.1f",s(i),m(j)),...
                'VerticalAlignment', 'middle', 'HorizontalAlignment', 'center');
        end
    end
